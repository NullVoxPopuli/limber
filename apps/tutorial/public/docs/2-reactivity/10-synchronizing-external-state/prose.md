...
- `@tracked` data may not be mutated / set in these functions. Setting `@tracked` data causes other parts of the UI to re-render, and functions auto-track. The function would detect the change to that tracked-data, re-run, and set the data again, causing an infinite rendering loop.
