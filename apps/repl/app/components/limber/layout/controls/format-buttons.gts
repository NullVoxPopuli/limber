...
class Option extends Component<{
  ...
  <template>
    <@item
      ...
      title={{@description}}
      class="relative -my-1 px-2 py-1 text-left ring-ember-brand drop-shadow-md transition duration-150 ease-in-out hover:drop-shadow-xl focus:rounded focus:ring-4 focus:outline-none focus-visible:rounded focus-visible:outline-none sm:text-sm
        {{this.active @value}}"
      ...attributes
    >
      {{yield}}
