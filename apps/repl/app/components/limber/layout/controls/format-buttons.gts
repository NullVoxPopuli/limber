...
export const FormatButtons: TOC<object> = <template>
  ...
  <style>
    ...
    box-shadow: 0 0px 4px rgba(255,255,255,0.4); } button:focus-visible, button:focus { z-index: 1;
    } }
  </style>
...
class Option extends Component<{
  ...
  <template>
    <@item
      ...
      title={{@description}}
      class="relative px-2 py-1 -my-1 text-left transition ease-in-out duration-150 sm:text-sm drop-shadow-md hover:drop-shadow-xl focus:ring-4 focus-visible:outline-none ring-ember-brand focus:outline-none focus-visible:rounded focus:rounded
        {{this.active @value}}"
      ...attributes
    >
      {{yield}}
