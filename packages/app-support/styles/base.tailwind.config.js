...
export function config(importMeta) {
  const selfRoot = url.fileURLToPath(new URL('.', importMeta.url));
  // Should probably change this to require-resolve, but we only have one
  const uiRoot = path.join(repoRoot, 'packages/app-support/limber-ui/addon');

  module.exports = {
    content: [
      `${selfRoot}/app/**/*.{js,ts,hbs,gjs,gts}`,
      `${selfRoot}/public/**/*.{md,gjs,gts}`,
